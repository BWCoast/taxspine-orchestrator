...
class TestSystemDiagnosticsEnhancedUI:
    ...
    def test_seven_tile_grid_in_html(self, html: str) -> None:
        """All seven tile sections (including new Prices DB) are present."""
        for tile_id in ('diag-lots', 'diag-prices', 'diag-pricesdb', 'diag-jobs', 'diag-dedup', 'diag-workspace', 'diag-disk'):
            assert f'id="{tile_id}"' in html, f"Missing tile: {tile_id}"
...
# ── TestDiagnosticsPricesDbSection ────────────────────────────────────────────


class TestDiagnosticsPricesDbSection:
    """GET /diagnostics prices section includes prices_db sub-key (ADR-0016)."""

    def test_prices_section_has_prices_db_key(self) -> None:
        client = _make_client()
        data = client.get("/diagnostics").json()
        assert "prices" in data
        assert "prices_db" in data["prices"], "prices.prices_db key missing"

    def test_prices_db_exists_false_when_no_db(self, tmp_path: Path) -> None:
        """prices_db.exists is False when prices.db does not exist."""
        client = _make_client()
        missing = tmp_path / "no_prices.db"
        with patch("taxspine_orchestrator.main.settings") as mock_s:
            mock_s.PRICES_DIR   = tmp_path
            mock_s.PRICES_DB    = missing
            mock_s.LOT_STORE_DB = tmp_path / "lots.db"
            mock_s.DEDUP_DIR    = tmp_path
            mock_s.ORCHESTRATOR_KEY = ""
            data = client.get("/diagnostics").json()
        db_info = data["prices"]["prices_db"]
        assert db_info.get("exists") is False

    def test_prices_db_exists_true_with_row_count(self, tmp_path: Path) -> None:
        """prices_db.exists is True and row_count is present when db exists."""
        import sqlite3
        db = tmp_path / "prices.db"
        con = sqlite3.connect(str(db))
        con.execute(
            "CREATE TABLE daily_prices "
            "(symbol TEXT, date TEXT, currency TEXT, price TEXT, "
            "source TEXT, fetched_at TEXT, PRIMARY KEY (symbol,date,currency))"
        )
        con.execute(
            "INSERT INTO daily_prices VALUES (?,?,?,?,?,?)",
            ("BTC", "2025-01-01", "NOK", "500000", "test", "2025-01-02T00:00:00"),
        )
        con.commit()
        con.close()
        ...
        with patch("taxspine_orchestrator.main.settings") as mock_s:
            ...
            data = client.get("/diagnostics").json()
        db_info = data["prices"]["prices_db"]
        assert db_info.get("exists") is True
        assert db_info.get("row_count") == 1
        assert db_info.get("last_fetched_at") == "2025-01-02T00:00:00"
        assert isinstance(db_info.get("size_kb"), (int, float))

    def test_prices_db_error_captured_gracefully(self, tmp_path: Path) -> None:
        """A corrupt prices.db produces an error key, not a 500."""
        db = tmp_path / "prices.db"
        db.write_bytes(b"not a sqlite database")  # corrupt
        ...
        assert r.status_code == 200
        db_info = r.json()["prices"]["prices_db"]
        assert "error" in db_info or db_info.get("exists") is True  # graceful
