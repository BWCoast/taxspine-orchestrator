...
SEC-04  Dockerfile build echoes ALTANA_SHA/ALTANA_TAG version to CI logs
...
class TestSEC04DockerfileVersionEcho:
    """SEC-04: ALTANA_SHA and ALTANA_TAG must not be echoed to build output;
    version metadata must be stored as image LABEL instead."""

    def test_altana_version_echo_removed(self):
        """The `echo '# tax-nor tag: ...'` line must be removed from RUN steps."""
        src = _dockerfile()
        assert 'echo "# tax-nor tag: ${ALTANA_TAG}' not in src, (
            "SEC-04: version echo must be removed from RUN build steps"
        )

    def test_altana_label_present(self):
        """Version info must be stored as image LABEL metadata."""
        src = _dockerfile()
        assert "LABEL" in src and "altana" in src.lower(), (
            "SEC-04: ALTANA_TAG/SHA must be stored as LABEL, not echoed"
        )

    def test_label_references_altana_tag(self):
        """LABEL directive must reference ALTANA_TAG (or altana.tag)."""
        src = _dockerfile()
        assert "altana.tag" in src or 'LABEL altana' in src, (
            "SEC-04: LABEL directive must capture ALTANA_TAG as altana.tag"
        )

    def test_label_references_altana_sha(self):
        """LABEL directive must reference ALTANA_SHA (or altana.sha)."""
        src = _dockerfile()
        assert "altana.sha" in src or 'LABEL altana' in src, (
            "SEC-04: LABEL directive must capture ALTANA_SHA as altana.sha"
        )

    def test_altana_arg_declarations_still_present(self):
        """ALTANA_TAG and ALTANA_SHA ARG declarations must remain for cache-busting."""
        src = _dockerfile()
        assert "ARG ALTANA_TAG=" in src, "ARG ALTANA_TAG must still be declared"
        assert "ARG ALTANA_SHA=" in src, "ARG ALTANA_SHA must still be declared"
