...
# unknown formats (Kraken, NBX, legacy Bybit UTA/Fund/Withdraw) are caught separately.
try:
...
_SNIFF_TO_SOURCE_TYPE: dict[str, CsvSourceType] = {
    ...
    "Uphold CSV": CsvSourceType.UPHOLD_CSV,
    "Bybit Activity CSV": CsvSourceType.BYBIT_ACTIVITY_CSV,
    "Binance Transaction CSV": CsvSourceType.BINANCE_TRANSACTION_CSV,
}
...
class JobService:
    ...
    def _run_subprocess_tracked(
        ...
        *,
        ...
        if _is_real:
            ...
            #
            # PYTHONUTF8=1 forces the *child* Python interpreter to use UTF-8
            # for all I/O (sys.stdout, sys.stderr).  This is the root fix for
            # the UnicodeEncodeError: tax CLIs emit box-drawing chars (═, ─, —)
            # via print(); on Windows a child process writing to a pipe inherits
            # the system locale (cp1252) for its OWN stdout encoding regardless
            # of what encoding= we pass to Popen here.  PYTHONUTF8=1 overrides
            # that.  The encoding='utf-8' on Popen tells the *parent* how to
            # decode the bytes it reads back from the pipe — both sides must
            # agree on UTF-8 for the round-trip to work correctly.
            _child_env = dict(os.environ)
            _child_env["PYTHONUTF8"] = "1"
            _child_env["PYTHONIOENCODING"] = "utf-8"  # belt-and-suspenders: explicit I/O encoding
            ...
            _child_env = dict(os.environ)
            _child_env["PYTHONUTF8"] = "1"
            _child_env["PYTHONIOENCODING"] = "utf-8"  # belt-and-suspenders
    ...
    def start_job_execution(self, job_id: str) -> Job | None:
        ...
        *would* be executed but does not call any subprocess.  The job
        ...
        try:
            ...
            if job.input.valuation_mode == ValuationMode.DB:
                # ADR-0016: prices.db mode — verify the database exists.
                # If absent, log a warning but continue; the CLI will fall
                # back to dummy valuation and flag has_unresolved rather
                # than aborting the job.
                _db_path = settings.PRICES_DB
                if _db_path is not None and _db_path.is_file():
                    log_lines.append(
                        f"[prices] valuation_mode=db: using {_db_path}"
                    )
                else:
                    log_lines.append(
                        "[prices] WARNING: valuation_mode=db but prices.db not found "
                        f"at {_db_path}. Run POST /prices/collect to populate it. "
                        "Proceeding without DB prices — expect has_unresolved."
                    )

            elif job.input.valuation_mode == ValuationMode.PRICE_TABLE:
            ...
            # Workspace isolation: resolve the lot store for this job's workspace.
            _lot_store = JobService._resolve_lot_store(job.input.workspace_id)
                ...
                if (
                    ...
                    for spec in _resolved_csv_files:
                        ...
                        csv_cmd = self._build_csv_command(
                            ...
                            lot_store=_lot_store,  # B4: cross-year carry-forward
                        )
                        log_lines.append(f"$ {' '.join(str(c) for c in csv_cmd)}")
            ...
            if _valuation_mode == "db":
                _price_source = "prices_db"
            elif _valuation_mode == "price_table":
                _price_source = "price_table_csv"
            else:
                _price_source = "dummy"
            ...
            # B1: CLI not on PATH — surface a clear actionable message instead of
            # "unexpected error: [Errno 2] No such file or directory: 'taxspine-...'".
            # Root cause: orchestrator venv lacks tax_spine (pip install -e . in Project F).
            missing = exc.filename or str(exc)
            log_lines.append(f"  FileNotFoundError: {exc}")
            return self._fail_job(
                job_id,
                error=(
                    f"Tax engine CLI not found: {missing!r}. "
                    "Ensure tax_spine is installed in the same Python environment as the "
                    "orchestrator: run 'pip install -e .' in the Project F directory."
                ),
                log_lines=log_lines,
                output_dir=output_dir,
            )
    ...
    def _build_xrpl_command(
        ...
        *,
        ...
        # Price source: DB mode, price_table mode, or blockchain-scanner backbone CSV.
        if job_input.valuation_mode == ValuationMode.DB:
            # B3: was missing — XRPL jobs silently ran dummy valuation when mode=db.
            # ADR-0016: pass prices.db path so CLI applies PriceSource.DB.
            _db = settings.PRICES_DB
            if _db is not None and _db.is_file():
                cmd.extend(["--prices-db", str(_db)])
        elif (
        ...
        # Workspace isolation: explicit lot_store overrides settings.LOT_STORE_DB.
        # When lot_store is None (direct call without workspace context) fall back
        # to the global setting so existing callers are unaffected.
        _effective_lot_store = lot_store if lot_store is not None else settings.LOT_STORE_DB
        if _effective_lot_store is not None:
            cmd.extend(["--lot-store", str(_effective_lot_store)])
    ...
    def _build_csv_command(
        ...
        *,
        ...
        elif csv_spec.source_type == CsvSourceType.UPHOLD_CSV:
            cmd.extend(["--input", csv_spec.path, "--source-type", "uphold_csv"])
        elif csv_spec.source_type == CsvSourceType.BYBIT_ACTIVITY_CSV:
            cmd.extend(["--input", csv_spec.path, "--source-type", "bybit_activity_csv"])
        elif csv_spec.source_type == CsvSourceType.BINANCE_TRANSACTION_CSV:
            cmd.extend(["--input", csv_spec.path, "--source-type", "binance_transaction_csv"])
        else:
        ...
        if job_input.valuation_mode == ValuationMode.DB:
            # ADR-0016: pass prices.db path; CLI applies PriceSource.DB.
            _db = settings.PRICES_DB
            if _db is not None and _db.is_file():
                cmd.extend(["--prices-db", str(_db)])
        elif (
        ...
        if job_input.country == Country.NORWAY:
            ...
            if job_input.unlinked_transfer_out_policy != "skip":
                cmd.extend(["--unlinked-transfer-out-policy", job_input.unlinked_transfer_out_policy])
            # B4: lot-store enables cross-year carry-forward in per-file mode.
            # Explicit lot_store overrides settings.LOT_STORE_DB (workspace isolation).
            # UK (S104) rebuilds the pool from scratch each year — no lot-store flag.
            _effective_lot_store = lot_store if lot_store is not None else settings.LOT_STORE_DB
            if _effective_lot_store is not None:
                cmd.extend(["--lot-store", str(_effective_lot_store)])
    ...
    _NOR_MULTI_SOURCE_TYPE: dict[CsvSourceType, str] = {
        ...
        CsvSourceType.UPHOLD_CSV: "uphold",
        # BYBIT_ACTIVITY_CSV and BINANCE_TRANSACTION_CSV not listed here;
        # nor_multi does not yet have their source types in SUPPORTED_SOURCES.
        # They fall back to PER_FILE mode automatically.
    }
    ...
    @staticmethod
    def _resolve_lot_store(workspace_id: str | None) -> Path | None:
        """Return the workspace-specific FIFO lot store path.

        Default workspace (workspace_id=None) → settings.LOT_STORE_DB.
        Named workspace → DATA_DIR/workspaces/{workspace_id}/lots.db.
        The workspace directory is created lazily when a named workspace is used.
        """
        if workspace_id is None:
            return settings.LOT_STORE_DB
        ws_dir = settings.DATA_DIR / "workspaces" / workspace_id
        ws_dir.mkdir(parents=True, exist_ok=True)
        return ws_dir / "lots.db"

    @staticmethod
    def _dedup_store_path(source_slug: str, workspace_id: str | None = None) -> Path:
        ...
        if workspace_id is None:
            ...
            dedup_dir = settings.DATA_DIR / "workspaces" / workspace_id / "dedup"
            dedup_dir.mkdir(parents=True, exist_ok=True)
        safe = re.sub(r"[^A-Za-z0-9_-]", "_", source_slug)
        resolved = (dedup_dir / f"{safe}.db").resolve()
        try:
            resolved.relative_to(dedup_dir.resolve())
        except ValueError:
            raise ValueError(
                f"Resolved dedup path {resolved!r} escapes dedup directory — "
