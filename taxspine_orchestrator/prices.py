...
_KRAKEN_USD_PAIR: dict[str, str] = {
    ...
    "ADA":  "ADAUSD",
    "LTC":  "LTCUSD",
    "HBAR": "HBARUSD",  # Hedera Hashgraph — listed on Kraken
    "CSPR": "CSPRUSD",  # Casper Network — listed on Kraken; Binance fallback if unavailable
    "FLR":  "FLRUSD",   # Flare Network — listed on Kraken; Binance fallback if unavailable
}
...
@router.post("/collect", tags=["prices"])
def collect_prices(target_date: str | None = None) -> dict:
    """Run the daily price collection job against prices.db (ADR-0016).

    Calls ``PriceCollectionJob.run_or_backfill()``:

    - If no prior run exists, or if a gap is detected since the last run,
      backfills the missing date range in a single CoinGecko API call.
    - Otherwise calls ``run()`` for yesterday (or ``target_date`` when given).

    The prices.db is created at ``settings.PRICES_DB`` if it does not exist.

    Parameters
    ----------
    target_date:
        Optional ISO 8601 date string (``YYYY-MM-DD``) for the collection
        target.  When absent, defaults to yesterday UTC.

    Returns
    -------
    dict with keys:
        - ``fetched``        — number of price entries written
        - ``skipped``        — already-present entries skipped
        - ``failed_count``   — entries that could not be fetched
        - ``failed``         — list of ``"SYMBOL/CURRENCY"`` failure keys
        - ``is_healthy``     — true when failure rate is within threshold
        - ``date_collected`` — date range string (``"YYYY-MM-DD"`` or ``"start..end"``)
        - ``db_path``        — absolute path to the prices.db used
    """
    import datetime as _dt

    from tax_spine.pricing.collection_job import PriceCollectionJob
    from tax_spine.pricing.norges_bank_fx import NorgesBankFxFetcher
    from tax_spine.pricing.price_store import PriceStore
    from tax_spine.pricing import COINGECKO_ASSET_ID_MAP

    db_path = settings.PRICES_DB
    db_path.parent.mkdir(parents=True, exist_ok=True)

    store = PriceStore(str(db_path))

    # Include workspace-registered XRPL assets in the asset map.
    asset_map = dict(COINGECKO_ASSET_ID_MAP)
    if _workspace_assets_provider is not None:
        for spec in (_workspace_assets_provider() or []):
            symbol = spec.split(".")[0].upper()
            if symbol and symbol not in asset_map:
                asset_map[symbol] = symbol.lower()  # best-effort CoinGecko slug

    import requests as _requests
    session = _requests.Session()

    fx_fetcher = NorgesBankFxFetcher(session=session)

    job = PriceCollectionJob(
        store=store,
        asset_id_map=asset_map,
        session=session,
        fx_fetcher=fx_fetcher,
    )

    parsed_target: _dt.date | None = None
    if target_date is not None:
        try:
            parsed_target = _dt.date.fromisoformat(target_date)
        except ValueError as exc:
            from fastapi import HTTPException as _HTTPException
            raise _HTTPException(
                status_code=422,
                detail=f"Invalid target_date format (expected YYYY-MM-DD): {exc}",
            ) from exc

    result = job.run_or_backfill(parsed_target)

    # PriceCollectionResult has: fetched, skipped, failed (dict[symbol, reason]),
    # is_healthy (property), date_collected. There is no failed_count or failed_detail.
    return {
