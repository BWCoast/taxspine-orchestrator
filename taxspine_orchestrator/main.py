...
from .storage import SqliteJobStore, WorkspaceStore, WorkspacePaths
...
def _get_ws_store(workspace_id: str | None) -> "WorkspaceStore":
    """Return the WorkspaceStore for workspace_id.

    Returns the global singleton for the default workspace (workspace_id=None).
    For named workspaces, creates a per-request WorkspaceStore pointing at
    DATA_DIR/workspaces/{workspace_id}/workspace.json.
    """
    if workspace_id is not None and not _WORKSPACE_ID_RE_MAIN.match(workspace_id):
...
app.include_router(ai_router,         dependencies=[Depends(_require_key)])
...
app.include_router(bot_router,        dependencies=[Depends(_require_key)])
app.include_router(workspaces_router, dependencies=[Depends(_require_key)])
...
def list_jobs(
    ...
    workspace: Optional[str] = Query(
        default=None,
        description=(
            "Filter jobs by workspace ID. "
            "Pass a workspace ID (e.g. 'friend') to see only that workspace's jobs. "
            "Pass '__default__' to see only default-workspace jobs (no named workspace). "
            "Omit to see all jobs regardless of workspace."
        ),
    ),
) -> list[Job]:
    ...
    """
    # Workspace filtering: None = all jobs; "__default__" = default workspace only;
    # any other string = named workspace only.
    _filter_workspace = workspace is not None
    _workspace_id_filter: str | None = None if workspace == "__default__" else workspace
    jobs = _job_service.list_jobs(
        status=status, country=country, query=query,
        limit=limit, offset=offset, after_id=after_id,
        filter_workspace=_filter_workspace,
        workspace_id=_workspace_id_filter,
    )
    total = _job_service.count_jobs(
        status=status, country=country, query=query,
        filter_workspace=_filter_workspace,
        workspace_id=_workspace_id_filter,
    )
...
async def upload_csv(
    ...
    **Excel files (.xlsx / .xls) are not accepted.**  Please open your file
    ...
    if register:
        _get_ws_store(workspace_id).add_csv(spec)
...
def purge_workspace(
    ...
    **Recommended retention schedule:**
    ...
    """
    store = _get_ws_store(workspace_id)
    ws = store.load()
    files_removed = 0
    if delete_files:
...
_WORKSPACE_ID_RE_MAIN = re.compile(r"^[a-z][a-z0-9_-]{0,37}$")
...
def add_workspace_xrpl_asset(
    body: AddXrplAssetRequest,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    ...
    """
    return _get_ws_store(workspace_id).add_xrpl_asset(body.spec)
...
def remove_workspace_xrpl_asset(
    spec: str,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Remove an XRPL token asset spec from the workspace."""
    return _get_ws_store(workspace_id).remove_xrpl_asset(spec)


# ── Workspace labels ──────────────────────────────────────────────────────────

class SetLabelRequest(BaseModel):
    """Request body for PUT /workspace/labels/* endpoints."""
    label: str = Field(description="Human-readable display name (1–80 chars).")
...
@app.put("/workspace/labels/wallet/{address}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def set_wallet_label(
    address: str,
    body: SetLabelRequest,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Set a human-readable display name for an XRPL wallet address.

    The label is purely cosmetic — it is shown in the dashboard in place of the
    raw r-address.  The address is stored as-is; no XRPL validation is performed
    here so that labels can be set for any address string.
    """
    return _get_ws_store(workspace_id).set_wallet_label(address, body.label)


@app.delete("/workspace/labels/wallet/{address}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def remove_wallet_label(
    address: str,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Remove the display label for a wallet address (no-op if not set)."""
    return _get_ws_store(workspace_id).remove_wallet_label(address)


@app.put("/workspace/labels/asset/{spec:path}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def set_asset_label(
    spec: str,
    body: SetLabelRequest,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Set a human-readable display name for an asset identifier.

    *spec* is the canonical asset string as it appears in the tax output —
    e.g. ``'SOLO.rHXuEaRYZBzZzb4vDiJFi8KRpU2mQhBpL'`` or
    ``'NFT:000803E88ACAA32313BB87F729EF7DA56BD13C57BE46E3C805A7DF630480F0D5'``.
    Exact-match lookup; takes priority over nft_collection_labels.
    """
    return _get_ws_store(workspace_id).set_asset_label(spec, body.label)


@app.delete("/workspace/labels/asset/{spec:path}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def remove_asset_label(
    spec: str,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Remove the display label for an asset identifier (no-op if not set)."""
    return _get_ws_store(workspace_id).remove_asset_label(spec)


@app.put("/workspace/labels/nft-collection/{prefix:path}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def set_nft_collection_label(
    prefix: str,
    body: SetLabelRequest,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Set a human-readable collection name for all NFTs sharing an issuer.

    *prefix* is the first 48 hex characters of the NFTokenID
    (flags[8] + transfer_fee[8] + issuer_account_id[40]).  All NFTs whose
    NFTokenID starts with this prefix will display the collection label in the
    dashboard when no exact asset_label is set for that specific NFT.
    """
    prefix = prefix.upper()
    if len(prefix) != 48 or not all(c in "0123456789ABCDEF" for c in prefix):
        raise HTTPException(status_code=422, detail="prefix must be exactly 48 uppercase hex characters")
    return _get_ws_store(workspace_id).set_nft_collection_label(prefix, body.label)


@app.delete("/workspace/labels/nft-collection/{prefix:path}", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def remove_nft_collection_label(
    prefix: str,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Remove the display label for an NFT collection (no-op if not set)."""
    return _get_ws_store(workspace_id).remove_nft_collection_label(prefix.upper())
...
def add_workspace_csv(
    ...
    return _get_ws_store(workspace_id).add_csv(CsvFileSpec(path=str(csv_path), source_type=source_type_enum))


@app.delete("/workspace/csv", response_model=WorkspaceConfig, tags=["workspace"], dependencies=[Depends(_require_key)])
def remove_workspace_csv(
    body: AddCsvRequest,
    workspace_id: str | None = Query(None, description="Named workspace (omit for default)"),
) -> WorkspaceConfig:
    """Remove a CSV file path from the workspace."""
    return _get_ws_store(workspace_id).remove_csv(body.path)
...
async def run_workspace_report(body: WorkspaceRunRequest, background_tasks: BackgroundTasks, workspace_id: str | None = Query(None, description="Named workspace (omit for default)")) -> Job:
    ...
    """
    ws = _get_ws_store(workspace_id).load()
...
async def get_alerts(
    ...
    workspace: Optional[str] = Query(
        default=None,
        description=(
            "Filter review alerts to a specific workspace. "
            "Pass '__default__' for the default workspace, a workspace ID for named workspaces, "
            "or omit to scan all workspaces."
        ),
    ),
) -> list[dict]:
    ...
    # Scope to workspace when caller provides one (workspace isolation).
    _alert_filter_ws = workspace is not None
    _alert_ws_id: str | None = None if workspace == "__default__" else workspace
    recent_jobs = _job_store.list(
        limit=limit,
        filter_workspace=_alert_filter_ws,
        workspace_id=_alert_ws_id,
    )
...
async def get_diagnostics() -> dict:
    ...
    def _prices_section() -> dict:
        ...
        csv_info: dict = {"csv_count": 0, "combined_csvs": []}
        if pdir.is_dir():
            try:
        ...
        # ADR-0016: prices.db stats — row count and last collection timestamp.
        # Use Path(str(...)) so that a mocked settings object never leaks a
        # non-serialisable value into the JSON response.
        db_info: dict
        try:
            _db_path = Path(str(settings.PRICES_DB))
            if not _db_path.is_file():
                db_info = {"exists": False}
            else:
                try:
                    _size_kb: float | None = round(_db_path.stat().st_size / 1024, 1)
                except OSError:
                    _size_kb = None
                try:
                    import sqlite3 as _sqlite3  # noqa: PLC0415
                    _con = _sqlite3.connect(f"file:{_db_path}?mode=ro", uri=True)
                    _row_count = int(_con.execute(
                        "SELECT COUNT(*) FROM daily_prices"
                    ).fetchone()[0])
                    _last_row = _con.execute(
                        "SELECT MAX(fetched_at) FROM daily_prices"
                    ).fetchone()[0]
                    _con.close()
                    db_info = {
                        "exists":          True,
                        ...
                        "last_fetched_at": str(_last_row) if _last_row else None,
                    }
                except Exception as _exc:  # noqa: BLE001
                    db_info = {"exists": True, "size_kb": _size_kb, "error": str(_exc)}
        except Exception as _exc:  # noqa: BLE001
            db_info = {"error": str(_exc)}

        return {**csv_info, "prices_db": db_info}
...
async def get_dashboard_sidecar(
    ...
    """Return the dashboard_summary.json sidecar for a completed Norway tax year.

    Written alongside the RF-1159 JSON by the ``taxspine-nor-report`` CLI.
    Requires ``PERSONAL_REPORTS_DIR`` to be set in the orchestrator environment.

    Response fields (all amounts are string-formatted NOK, 2 decimal places):
      ``schema_version``, ``generated_at``, ``year``,
      ``total_gains_nok``, ``total_losses_nok``, ``net_gain_nok``,
      ``income_nok``, ``tax_owed_nok``.

    Returns 404 when the sidecar has not been generated or the directory is
    not configured.

    When ``workspace_id`` is provided, looks in
    ``PERSONAL_REPORTS_DIR/{workspace_id}/{year}/dashboard_summary.json``.
    For the default workspace, looks in ``PERSONAL_REPORTS_DIR/{year}/dashboard_summary.json``.
    """
    if not settings.PERSONAL_REPORTS_DIR:
        raise HTTPException(
            status_code=404,
            detail="PERSONAL_REPORTS_DIR is not configured — sidecar not available",
        )
    ...
    if not sidecar_path.is_file():
        raise HTTPException(
            status_code=404,
            detail=f"No dashboard sidecar found for {year} at {sidecar_path}",
        )
