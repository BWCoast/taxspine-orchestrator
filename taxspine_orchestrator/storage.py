...
class WorkspacePaths:
    """Resolved filesystem paths for a named workspace.

    All workspace-specific data lives under DATA_DIR/workspaces/{workspace_id}/.
    The default workspace (workspace_id=None) uses the global paths from settings.

    Attributes:
        lots_db:        FIFO lot store path (None when no lot store configured).
        dedup_dir:      Directory for per-source dedup SQLite databases.
        workspace_json: Per-workspace config JSON file.
    """
    ...
    def resolve(
        ...
        *,
        ...
        """Return paths for the given workspace_id (None = default workspace).

        Named workspaces are stored under DATA_DIR/workspaces/{workspace_id}/.
        Directories are NOT created here — callers should create them lazily.
        """
        if workspace_id is None:
            return WorkspacePaths(
                lots_db=lots_db,
                dedup_dir=dedup_dir,
                workspace_json=data_dir / "workspace.json",
            )
        ws_dir = data_dir / "workspaces" / workspace_id
        return WorkspacePaths(
            lots_db=ws_dir / "lots.db",
            dedup_dir=ws_dir / "dedup",
...
class InMemoryJobStore:
    ...
    def ping(self) -> None:
        """No-op — in-memory store is always reachable."""
    ...
    def list(
        ...
        *,
        ...
        if filter_workspace:
            jobs = [j for j in jobs if j.input.workspace_id == workspace_id]
    ...
    def delete_by_workspace(self, workspace_id: str | None) -> int:
        """Delete all jobs belonging to *workspace_id*. workspace_id=None → default workspace.

        Returns the count of deleted jobs.
        """
        to_delete = [
            jid for jid, job in self._jobs.items()
            if job.input.workspace_id == workspace_id
        ]
    ...
    # ── SqliteJobStore compatibility shims (used by some tests) ──────────────

    def _upsert(self, job: Job) -> Job:
        """Directly replace a job record — mirrors SqliteJobStore._upsert.

        Used by tests that need to inject arbitrary job state (e.g. old
        ``updated_at`` timestamps) without going through the public API.
        """
        self._jobs[job.id] = job
        return job

    def log_deletion(self, job_id: str, files_removed: int = 0) -> None:
        """LC-11 shim: append a deletion event to the in-memory audit log."""
    ...
    def list_deletions(self, limit: int = 100) -> list[dict]:
        """LC-11 shim: return recent deletion audit log entries (newest first)."""
        entries = list(reversed(self._deletion_log))
...
class SqliteJobStore:
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(str(self._db_path)) as conn:
            ...
            # Workspace isolation migration: add workspace_id column if absent.
    ...
    def add(self, job: Job) -> Job:
        with self._lock, self._connect() as conn:
            conn.execute(
                ...
                "(id, status, country, case_name, created_at, data, workspace_id) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    job.id,
                    job.status.value,
    ...
    def delete_by_workspace(self, workspace_id: str | None) -> int:
        ...
        with self._lock, self._connect() as conn:
            if workspace_id is None:
                cursor = conn.execute(
                    "DELETE FROM jobs WHERE workspace_id IS NULL"
                )
            else:
                cursor = conn.execute(
...
class WorkspaceStore:
    ...
    def set_wallet_label(self, address: str, label: str) -> WorkspaceConfig:
        """Set or update the display label for a wallet address."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"wallet_labels": {**cfg.wallet_labels, address: label}}
            )
            self._save_locked(cfg)
            return cfg

    def remove_wallet_label(self, address: str) -> WorkspaceConfig:
        """Remove the display label for a wallet address (no-op if absent)."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"wallet_labels": {k: v for k, v in cfg.wallet_labels.items() if k != address}}
            )
            self._save_locked(cfg)
            return cfg

    def set_asset_label(self, spec: str, label: str) -> WorkspaceConfig:
        """Set or update the display label for an asset identifier."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"asset_labels": {**cfg.asset_labels, spec: label}}
            )
            self._save_locked(cfg)
            return cfg

    def remove_asset_label(self, spec: str) -> WorkspaceConfig:
        """Remove the display label for an asset identifier (no-op if absent)."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"asset_labels": {k: v for k, v in cfg.asset_labels.items() if k != spec}}
            )
            self._save_locked(cfg)
            return cfg

    def set_nft_collection_label(self, prefix: str, label: str) -> WorkspaceConfig:
        """Set or update the display label for an NFT collection (48-char prefix)."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"nft_collection_labels": {**cfg.nft_collection_labels, prefix: label}}
            )
            self._save_locked(cfg)
            return cfg

    def remove_nft_collection_label(self, prefix: str) -> WorkspaceConfig:
        """Remove the display label for an NFT collection (no-op if absent)."""
        with self._lock:
            cfg = self._load_locked()
            cfg = cfg.model_copy(
                update={"nft_collection_labels": {k: v for k, v in cfg.nft_collection_labels.items() if k != prefix}}
            )
            self._save_locked(cfg)
            return cfg
