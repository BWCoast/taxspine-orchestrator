...
class CsvSourceType(str, Enum):
    ...
    FIRI_CSV = "firi_csv"
    UPHOLD_CSV = "uphold_csv"
    BYBIT_ACTIVITY_CSV = "bybit_activity_csv"
    BINANCE_TRANSACTION_CSV = "binance_transaction_csv"
...
class ValuationMode(str, Enum):
    ...
    - ``DB`` — use prices.db SQLite store via ``--prices-db`` (ADR-0016).
    """
...
class JobInput(BaseModel):
    ...
    workspace_id: Optional[str] = Field(
        ...
        description=(
            "Named workspace for complete data isolation.  When set, FIFO lots, "
            "dedup records, and workspace config are stored in a dedicated "
            "subdirectory and never mixed with the default workspace or other "
            "named workspaces.  None = default workspace (your own data).  "
...
class WorkspaceConfig(BaseModel):
    ...
    wallet_labels: Dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Human-readable display names for XRPL wallet addresses, keyed by the "
            "full r-address. Used by the dashboard to show friendly names instead of "
            "raw addresses (e.g. 'rwvZ2Ad…' → 'Main wallet'). Purely cosmetic — "
            "no effect on tax calculation."
        ),
    )
    asset_labels: Dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Human-readable display names for asset identifiers, keyed by the "
            "canonical asset string. Covers fungible tokens "
            "('SOLO.rHXu…' → 'Solo') and individual NFTs "
            "('NFT:000803E8…' → 'Rare Pepe #42'). Exact-match lookup; "
            "falls back to nft_collection_labels then auto-formatting."
        ),
    )
    nft_collection_labels: Dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Human-readable collection names keyed on the 48-char NFTokenID prefix "
            "(hex flags[8] + transfer_fee[8] + issuer_account_id[40]). "
            "Labels every NFT minted by the same issuer when no exact asset_label exists."
        ),
    )
