...
| POST   | `/workspace/xrpl-assets`        | Register an XRPL IOU token for price tracking |
| DELETE | `/workspace/xrpl-assets/{spec}` | Unregister an XRPL IOU token                |
...
IOU tokens in the trust lines of registered workspace accounts are auto-discovered via
`account_lines` RPC at price-fetch time and included automatically.  Tokens held in
unregistered accounts, or tokens whose balance is currently zero (e.g. sold mid-year
but still needing a disposal-date price), require explicit registration via
`POST /workspace/xrpl-assets`.  See [XRPL IOU token registration](#xrpl-iou-token-registration).

---

## XRPL IOU token registration

XRPL accounts can hold IOU tokens issued by third parties — stablecoins, DEX
tokens, protocol tokens, and so on.  These tokens must appear with a NOK price
in the year-end wealth calculation (RF-1159 Formue).  Without a price, the
pipeline produces `formue = 0 NOK` for that token — an understatement that
surfaces as a filing-blocking `rf1159_warnings` entry.

### Why a price is needed

IOU token prices are not directly quoted in NOK.  The price engine derives them
from DEX trade data: `price_nok = (token / XRP DEX fill rate) × XRP_nok`.
If the engine cannot find any price through the four price tiers, it marks the
asset UNRESOLVED.  An UNRESOLVED asset at year-end sets `has_unresolved_prices`
in the pipeline summary and adds an entry to `rf1159_warnings`.  The job cannot
be filed as-is.

### Auto-discovery (no action needed for most tokens)

When you run `POST /prices/fetch` (or trigger it automatically via a job),
every account registered via `POST /workspace/accounts` has its XRPL trust lines
queried.  Every IOU token currently held in those trust lines is fetched and
added to `combined_nok_{year}.csv` automatically.  If all your IOU tokens are
in wallet accounts already registered in the workspace, you do not need to
register them manually.

### When manual registration is required

Register a token explicitly if:

- it is held in an account that is **not** registered in the workspace, or
- its balance is currently **zero** (sold mid-year) but you still need a
  disposal-date price for the transaction that closed the position.

```bash
# Register a token: SYMBOL.rIssuerAddress
curl -s -X POST http://localhost:8000/workspace/xrpl-assets \
  -H "Content-Type: application/json" \
  -d '{"spec": "SOLO.rHXuEaRoqhbTuHtLBnXKeHG53bAnkEfFX"}'

# Confirm the registration
curl -s http://localhost:8000/workspace | jq '.xrpl_assets'

# Remove a registration when no longer needed
curl -s -X DELETE \
  'http://localhost:8000/workspace/xrpl-assets/SOLO.rHXuEaRoqhbTuHtLBnXKeHG53bAnkEfFX'
```

The `spec` format is `SYMBOL.rIssuerAddress`.  The symbol is the currency code
as used on the XRPL DEX (3-character ASCII or hex-decoded); the issuer is the
full XRPL r-address of the token's issuer.  Examples:

| Token | Spec |
|-------|------|
| Sologenic SOLO | `SOLO.rHXuEaRoqhbTuHtLBnXKeHG53bAnkEfFX` |
| Ripple USD (RLUSD) | `RLUSD.rMxWRkh2o3tNHJFRRPCvSwJqMWQHJt3Z5j` |
| Bitstamp USD | `USD.rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B` |

Specs are validated on registration — the server rejects malformed r-addresses.

### What happens if a token stays UNRESOLVED

- `job.output.rf1159_warnings` lists the unresolved asset(s).
- The RF-1159 export contains a `warnings` array.
- `has_unresolved_prices` is set in the pipeline summary.
- The review queue blocks the "safe for filing" gate.

To resolve: add the missing token's year-end price directly to
`combined_nok_{year}.csv` (one row: `date,asset,price_nok`) using the qualified
`SYMBOL.rIssuerAddress` key, then re-run the job.
