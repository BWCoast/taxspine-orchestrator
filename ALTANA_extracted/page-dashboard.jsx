...
const Dashboard = ({ juris, year, setYear, addToast, wsName, userName, setRoute }) => {
  ...
  // ── Live data ──────────────────────────────────────────────────────────────
  const { data: portfolio }  = window.useApi(() => window.API.getPortfolio(year, true), [year]);
  const { data: sidecar }    = window.useApi(() => window.API.getSidecar(year),          [year]);
  const { data: review }     = window.useApi(() => window.API.getReviewSummary(year),    [year]);
  const { data: diags }      = window.useApi(() => window.API.getDiagnostics(),           []);
  // Workspace-scoped config — used for source count so named workspaces (e.g. a friend's
  // workspace) aren't blocked by the unscoped diagnostics endpoint returning 0.
  const { data: wsConfig }   = window.useApi(() => window.API.getWorkspace(),             []);
  const { data: recentJobs } = window.useApi(
    () => window.API.getJobs({ limit: 8, status: 'completed' }), []
  );
  const { data: lotMeta }    = window.useApi(() => window.API.getLotYear(year),           [year]);
  ...
  // ── Activity feed from recent completed jobs ──────────────────────────────
  const activity = recentJobs?.length
    ? recentJobs.map(j => ({
  ...
  // ── System health ─────────────────────────────────────────────────────────
  // Shared infrastructure (prices, dedup) read from global diagnostics — correct.
  // Workspace-specific rows (wallets, CSVs, active jobs) use workspace-scoped sources.
  const wsRunningJobs = recentJobs
    ? (window.API.getJobs ? null : 0)  // placeholder — use wsConfig + recentJobs below
    : null;
  const wsWallets  = wsConfig?.xrpl_accounts?.length ?? 0;
  const wsCsvs     = wsConfig?.csv_files?.length ?? 0;
  // Active jobs: count running jobs in THIS workspace from the all-jobs list.
  // recentJobs only fetches completed; load running separately.
  const { data: runningJobs } = window.useApi(
    () => window.API.getJobs({ limit: 50, status: 'running' }), []
  );
    ...
    ? [
        {
          ...
          last: diags.lots?.size_kb ? `${diags.lots.size_kb} KB` : 'no data',
        },
        {
          name: 'Price cache',
        ...
        },
        {
          ...
          last: diags.prices?.prices_db?.row_count
            ? `${(diags.prices.prices_db.row_count / 1000).toFixed(0)}k rows` : '—',
        },
        {
          name: 'Active jobs',
          status: wsRunning > 0 ? 'amber' : 'green',
          last: `${wsRunning} running`,
        },
        {
          name: 'Dedup store',
  ...
  const unlinkedCount     = review?.unlinked_transfer_jobs?.length ?? 0;
  const warningCount      = review?.total_warnings ?? 0;
  const hasPrices         = (diags?.prices?.combined_csvs?.length ?? 0) > 0
                         || diags?.prices?.prices_db?.exists;
  // Use workspace-scoped wsConfig (not diags.workspace which is unscoped).
  const sourceCount       = (wsConfig?.xrpl_accounts?.length ?? 0)
                          + (wsConfig?.csv_files?.length ?? 0);
  // Use workspace-scoped recentJobs — diags.jobs.completed counts across ALL workspaces.
  const hasCompletedJob   = (recentJobs?.length ?? 0) > 0;

  const checklistItems = [
    { done: true,                    label: 'Jurisdiction set' },
    { done: sourceCount > 0,         label: `${sourceCount} source${sourceCount !== 1 ? 's' : ''} connected` },
    { done: !!hasPrices,             label: 'Prices collected' },
    { done: missingBasisCount === 0, label: missingBasisCount > 0 ? `Resolve ${missingBasisCount} missing-basis assets` : 'Missing basis clear' },
    { done: warningCount === 0,      label: warningCount > 0 ? `Resolve ${warningCount} warning(s)` : 'All warnings resolved' },
    { done: hasCompletedJob,         label: juris === 'NO' ? 'Generate RF-1159 report' : 'Generate SA108 report' },
  ];
  const readinessPct = Math.round(checklistItems.filter(c => c.done).length / checklistItems.length * 100);
  ...
  const lotsLabel = totalLots != null
    ...
    : diags ? 'no lot data' : 'lots loading…';
