...
const Sidebar = ({ route, setRoute, collapsed }) => {
  ...
  const { data: review } = window.useApi(() => window.API.getReviewSummary(currentYear), []);
  // Workspace-scoped jobs for badge — must match what the Alerts page sees.
  // getDiagnostics().jobs.failed is global (all workspaces), so we count from
  // the workspace-scoped job list instead.
  ...
  const jobStatus   = jobsRunning > 0 ? 'amber' : 'green';
  ...
  return (
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      ...
      <div className="sidebar-footer">
        ...
        <div className="health-row"><span className={`dot ${jobStatus}`}/> Jobs · {jobsRunning} running</div>
      </div>
    </aside>
  );
};
...
const Topbar = ({ route, setRoute, juris, setJuris, theme, toggleTheme, openCmdK, collapsed, setCollapsed, wsName, userName }) => {
  const titles = {
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-right">
        ...
        <button className="icon-btn tip" data-tip="Alerts" onClick={() => setRoute('alerts')}>
          <Icon name="bell"/>
          <span className="pip"/>
        </button>
