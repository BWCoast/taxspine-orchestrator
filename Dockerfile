...
#   rm .gh_token
#
# Cache-busting: ALTANA_SHA is the HEAD commit SHA of the tax-nor repo,
...
# ALTANA_TAG: git ref to install — commit SHA, tag, or branch (default main).
# The CI workflow passes the HEAD SHA of tax-nor so builds are deterministic.
# ALTANA_SHA: same SHA, used only for Docker layer cache-busting.
ARG ALTANA_TAG=main
ARG ALTANA_SHA=unknown
...
LABEL altana.tag="${ALTANA_TAG}" altana.sha="${ALTANA_SHA}"
RUN --mount=type=secret,id=gh_token,required=false \
    TOKEN=$(cat /run/secrets/gh_token 2>/dev/null || echo "") && \
    if [ -n "$TOKEN" ]; then \
        pip install --no-cache-dir "git+https://${TOKEN}@github.com/BWCoast/tax-nor.git@${ALTANA_TAG}"; \
    else \
        pip install --no-cache-dir "git+https://github.com/BWCoast/tax-nor.git@${ALTANA_TAG}"; \
    fi
