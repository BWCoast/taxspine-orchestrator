...
const Sources = ({ addToast, setRoute, reloadWorkspace }) => {
  // reloadWorkspace — App-level reload; call after mutations so sharedWorkspace
  // propagates to Dashboard, Settings, and Sidebar immediately without waiting 10 s.
  ...
  // Helper: refresh both local Sources view AND App-level shared signal.
  const reloadAll = () => { reloadWs(); if (reloadWorkspace) reloadWorkspace(); };
    ...
    <>
      <div className="page-header">
        ...
        <div className="row" style={{ gap: 8 }}>
          <button className="btn" onClick={() => { reloadAll(); addToast({ title: 'Refreshed', msg: 'Workspace data reloaded' }); }}>
            ...
            {tab === 'bot' && (
              <div className="card-body">
                ...
                <div className="row" style={{ flexWrap: 'wrap', gap: 6 }}>
                  ...
                  <button className="btn sm" onClick={() => addToast({ title: 'Add XRPL asset', msg: 'Go to Settings → XRPL asset registry to add a token spec (SYMBOL.rIssuer format).', kind: 'info' })}>
...
// ── Tax Center ────────────────────────────────────────────────────────────────
const TaxCenter = ({ juris, setJuris, year, addToast, diags, sharedJobs, reloadJobs, reloadDiags }) => {
  // diags, sharedJobs, reloadJobs, reloadDiags — shared from App (10 s polling).
  ...
  const runJob = async () => {
    ...
    try {
      ...
      // Reload immediately so the new job appears in history + sidebar lights update
      // without waiting for the 10 s shared-signal poll.
      reloadJobs();

      // Poll every 3 s until terminal state.
      let _pollErrors = 0;
      const MAX_POLL_ERRORS = 20; // 20 × 3 s = 60 s of consecutive server errors before giving up
      _pollRef.current = setInterval(async () => {
        try {
          ...
          if (_pollErrors >= MAX_POLL_ERRORS) {
            clearInterval(_pollRef.current);
            setRunning(false);
            setRunError('Lost contact with server after 60 s — check if orchestrator is running.');
            addToast({ title: 'Poll timeout', msg: 'Server unreachable — job status unknown' });
          }
          // otherwise: transient error, keep polling
        }
      }, 3000);
  ...
  const collectPrices = async () => {
    ...
    try {
      ...
      // Prices appear in diagnostics (prices.db stats, combined CSV list) — refresh both.
      if (reloadDiags) reloadDiags();
      reloadJobs();
    } catch (e) {
              ...
              <div className="grid grid-2" style={{ gap: 12, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <select value={taxYear} onChange={e => setTaxYear(Number(e.target.value))}
                          style={{ width: '100%', padding: 6, borderRadius: 7, background: 'var(--bg-deep)', border: '1px solid var(--border)', color: 'var(--text)' }}>
                    {/* Fixed range: currentYear-1 down to 2017, independent of the global year picker */}
                    {Array.from({ length: currentYear - 1 - 2016 }, (_, i) => currentYear - 1 - i)
              ...
              <tbody>
                ...
                {jobs?.map(j => (
                  <tr key={j.id}>
                    ...
                    <td>{j.input?.country === 'norway' ? '🇳🇴 NO' : '🇬🇧 UK'}</td>
                    <td>
...
// ── Review Queue ──────────────────────────────────────────────────────────────
const ReviewQueue = ({ juris, year, addToast, review, reloadReview }) => {
  // review and reloadReview are shared from App (10 s polling, tracks selected year).
  const [tab, setTab] = React.useState('unlinked');
  const loading = !review;
  const reload  = reloadReview;
