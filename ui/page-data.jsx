...
// Prices page
// ─────────────────────────────────────────────────────────────────────────────
const Prices = ({ juris, year, addToast, diags, wsConfig, reloadDiags, reloadWorkspace }) => {
  // diags, wsConfig, reloadDiags, reloadWorkspace — shared from App (10 s polling).
  // year — globally selected tax year; used as the default for price collection.
  const ccy          = juris === 'NO' ? 'NOK' : 'GBP';
  const diagsLoading = !diags;
  const ws           = wsConfig;
  const reloadWs     = reloadWorkspace;

  const currentYear = new Date().getFullYear();
  // Local year for the collect action — independent of the global year selector.
  // Initialized from the global year so opening the page lands on the right year,
  // but the user can change it here without touching the Dashboard year picker.
  const [collectYear, setCollectYear] = React.useState(year ?? currentYear);
  // Keep in sync if the user changes the global year while on this page.
  React.useEffect(() => { if (year != null) setCollectYear(year); }, [year]);
  ...
  const collectNow = async () => {
    setCollecting(true);
    try {
      await window.API.collectPrices(collectYear);
      addToast({ title: 'Price collect started', msg: `Fetching ${collectYear} prices — reload in a moment` });
      ...
      <div className="page-header">
        ...
        <div className="row" style={{gap: 8}}>
          <div className="year-pill">
            <button className="arrow" onClick={() => setCollectYear(y => y - 1)} disabled={collecting}>
              <Icon name="chevron_left" size={12}/>
            </button>
            <span>{collectYear}</span>
            <button className="arrow" onClick={() => setCollectYear(y => y + 1)} disabled={collecting || collectYear >= currentYear}>
              <Icon name="chevron_right" size={12}/>
            </button>
          </div>
          <button className="btn primary" onClick={collectNow} disabled={collecting}>
            <Icon name="refresh" size={12}/> {collecting ? `Collecting ${collectYear}…` : 'Collect prices now'}
          </button>
        </div>
      </div>
          ...
          <div className="card-body">
            ...
            <div className="row" style={{marginTop: 12, gap: 6}}>
              <button className="btn sm" onClick={collectNow} disabled={collecting}>
                {collecting ? 'Collecting…' : `Refresh ${collectYear}`}
              </button>
...
// Charts page
// ─────────────────────────────────────────────────────────────────────────────
const Charts = ({ juris, diags }) => {
  // diags — shared from App (10 s polling).
  const D           = window.DEMO;
  const [showBotFills, setShowBotFills] = React.useState(false);
  const ccy    = juris === 'NO' ? 'NOK' : 'GBP';
  const fxRate = juris === 'NO' ? 1 : 0.078;
        ...
        <div style={{gridColumn: 'span 8'}}>
          <div className="card">
            <div className="card-header">
              <div className="card-title">
                Portfolio · {ccy}
                {totalNOK > 0 && <span className="meta">· 60d synthetic</span>}
              </div>
              {totalNOK > 0 && (
                <div className="row" style={{gap: 8}}>
                  <label className="row" style={{gap: 6, fontSize: 12, color: 'var(--text-muted)', cursor: 'pointer'}}
                         title="Bot fills overlay — UI placeholder, no data source yet">
                    <span className={`toggle ${showBotFills ? 'on' : ''}`}
                          onClick={() => setShowBotFills(v => !v)}/> Bot fills
                  </label>
                  <div className="tabs">
            ...
            <div style={{padding: 12}}>
              {totalNOK > 0
                ...
                : <div style={{height: 280, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-faint)', gap: 8}}>
                    <Icon name="charts" size={28}/>
                    <div style={{fontSize: 13}}>No portfolio data for {chartYear}</div>
                    <div style={{fontSize: 11.5}}>Run a tax job for {chartYear} first</div>
                  </div>
              }
            </div>
